...
Portable setup uses tools already on `PATH`, but admits only the exact Nub pin in `package.json` before Node discovery. It never upgrades Nub; see [portable recovery](docs/prerequisites.md#nub-admission-and-portable-recovery). Setup is safe to re-run after moving between dev environments and preserves machine-local pi auth, model choices, and local overrides. It updates the pi-env guidance block in `~/.pi/agent/AGENTS.md` and preserves content outside the managed block.
