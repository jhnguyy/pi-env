...
# structured capability map that serves as a jumping-off point for agents and
# users. It intentionally avoids duplicating doc content — each entry links to
...
# Extract provider list from README (between "API keys:" and next section)
PROVIDERS=$(sed -n '/^\*\*API keys:\*\*/,/^$/p' "$PI_PKG/README.md" | grep -E '^- ' | sed 's/^- //' | tr '\n' ', ' | sed 's/,[ ]*$//' | sed 's/,,/,/g')

# Extract built-in tool list
BUILTIN_TOOLS="read, bash, edit, write, grep, find, ls"

# Count theme color tokens
THEME_TOKENS=$(grep -c '| `' "$PI_DOCS/themes.md" 2>/dev/null || echo "51")

# Count settings
SETTINGS_COUNT=$(grep -c '| `' "$PI_DOCS/settings.md" 2>/dev/null || echo "~30")

# Count event types from extensions.md
EVENT_COUNT=$(grep -cE '^\#{3,4} [a-z_]+' "$PI_DOCS/extensions.md" 2>/dev/null || echo "~25")

# Count keybinding actions
...
> **Auto-generated from pi v${VERSION}** — run \`bash scripts/generate-capability-map.sh\` after version bumps.
...
- **Event interception** — block/modify tool calls, inject context, customize compaction (~${EVENT_COUNT} event types)
...
- **Inter-extension events** — \`pi.events\` shared bus
  ...
  **Doc:** [extensions.md](../node_modules/@mariozechner/pi-coding-agent/docs/extensions.md) — ${EXT_COUNT} working examples in [examples/extensions/](../node_modules/@mariozechner/pi-coding-agent/examples/extensions/)
...
- ${THEME_TOKENS} color tokens covering UI, markdown, syntax, diffs, thinking levels
...
| Customizable keys | \`keybindings.json\` | ${KEYBINDING_ACTIONS} rebindable actions |
...
${SETTINGS_COUNT} configurable options across: model/thinking, UI/display, compaction, retry, message delivery, terminal, shell, and resource paths.
...
${SDK_COUNT} SDK examples in [examples/sdk/](../node_modules/@mariozechner/pi-coding-agent/examples/sdk/)
...
All ${DOC_FILES} doc files in \`node_modules/@mariozechner/pi-coding-agent/docs/\`:
