...
describe("dependency policy", () => {
  ...
  it("limits the known baseline to the existing cross-extension imports", () => {
    ...
    expect(violations.map(({ from, to, rule }) => `${rule.name}: ${from} -> ${to}`).sort()).toEqual(
      [
        "review-no-cross-extension-imports: .pi/extensions/review/index.ts -> .pi/extensions/subagent/control.ts",
        "review-no-cross-extension-imports: .pi/extensions/review/index.ts -> .pi/extensions/subagent/execute.ts",
