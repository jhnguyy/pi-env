...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    description: [
      ...
      "",
      "Actions:",
      "  start   — Begin a new orchestration run. Creates ORCH_DIR (/tmp/orch-*), sets PI_BUS_SESSION.",
      "            Optional: repo enables git worktree isolation (separate branch per worker). Returns: runId, orchDir, busSession.",
      "",
      "  spawn   — Spawn one worker pane. Required: label (a-z, 0-9, hyphens).",
      "            Modes: command (raw shell) or model/tools/brief/prompt (pi spawner — orch builds the pi command).",
      "            Cannot mix modes. Injects PI_BUS_SESSION, PI_AGENT_ID, ORCH_DIR. Workers write to $ORCH_DIR/<label>.json.",
      "            If repo set on start: creates worktree on branch orch/<runId>/<label>. Optional: busChannel.",
      "            Returns: paneId, branch (if worktree), worktreePath (if worktree).",
      "",
      "  cleanup — Kill all panes, remove worktrees, delete ORCH_DIR. Branches preserved (listed in receipt).",
      "            Writes /tmp/orch-runs/<ts>-<runId>.json. Required: call after synthesis is complete.",
      "",
      "  status  — Show current run state: runId, workers, orchDir, busSession. Returns 'no active run' if not started.",
      "",
      "  wait    — Block until any spawned worker's busChannel fires. No args needed — orch knows channels from spawn.",
      "            Optional: timeout (default 300s). Requires at least one worker spawned with busChannel.",
      "",
      "Typical flow:",
