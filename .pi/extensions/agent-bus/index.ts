...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    description: [
      ...
      "",
      "Actions:",
      "  start     — Initialize bus session. Optional: session (string), agentId (string).",
      "              Sets PI_BUS_SESSION and PI_AGENT_ID for this process. Idempotent.",
      "  publish   — Write message to channel. Required: channel, message.",
      "              Optional: type (status|result|error|command), data (object).",
      "  subscribe — Register channels for notification. Required: channels (array).",
      "              Additive. Sets cursor to now — skips historical messages.",
      "  check     — Non-blocking. Count new messages on subscribed channels.",
      "  read      — Read all messages since cursor, advance cursor. Required: channel.",
      "              Implicitly subscribes (reads historical messages).",
      "  wait      — Block until new messages on any listed channel. Required: channels.",
      "              Optional: timeout (seconds, default 300). Implicitly subscribes.",
      "              Returns messages that triggered wake.",
      "",
