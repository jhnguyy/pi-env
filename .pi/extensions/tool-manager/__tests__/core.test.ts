...
const tools = [
  ...
  name,
  description: name === "notes" ? "team notes forgejo" : name,
  parameters: {},
  sourceInfo: { source: "x" } as any,
}));

describe("tool manager core", () => {
  it("uses the core profile after all tools are known and rejects an invalid configured default", () => {
    expect(profileTools("core", resolveConfig(), tools)).toEqual([
      "read",
      "bash",
      "edit",
      "write",
  ...
  it("expands requested names without forcing alwaysActive into removal inputs", () => {
    ...
    expect(
      setAdditive(["read"], expandRequestedEntries(["analysis"], cfg, tools), cfg, tools),
    ).toContain("notes");
    expect(setAdditive(["read", SEARCH_TOOL_NAME, "notes"], [], cfg, tools)).toEqual([
      "read",
      SEARCH_TOOL_NAME,
      "notes",
    ]);
  ...
  it("search is reliable and respects manual-only", () => {
    ...
    expect(
      searchTools("bounded complexity loads analysis", ["read"], resolveConfig(), tools).loaded,
    ).toEqual(["analyze"]);
    ...
    expect(searchTools("linear issue", ["read"], resolveConfig(), tools).loaded).not.toContain(
      "linear_create_issue",
    );
  });
  ...
  it("trigger matrix auto-activates analysis for coding sessions with code entities", () => {
    ...
    expect(triggerGroups({ text: "delegate this to subagent in background" }, true)).toContain(
      "delegation",
    );
    expect(triggerGroups({ text: "create a skill for this" }, true)).toContain("skills");
    expect(triggerGroups({ text: "use catching for JIT phrasing" }, true)).toContain(
      "catching-tests",
    );
    expect(triggerGroups({ text: "read previous session conversation" }, true)).toContain(
      "sessions",
    );
    expect(
      triggerGroups({ text: "Review this PR https://github.com/acme/widgets/pull/123" }, true),
    ).toContain("pr-review");
    ...
    expect(
      triggerGroups({ text: "Get comments from https://github.com/acme/widgets/pull/123" }, true),
    ).toContain("pr-review");
    ...
    expect(triggerGroups({ text: "find the Linear issue for this work" }, true)).toContain(
      "linear-read",
    );
  });
  ...
  it("keeps Linear write tools manual-only by default", () => {
    ...
    expect(config.manualOnly).toEqual(
      new Set(["linear_create_issue", "linear_update_issue", "linear_create_comment"]),
    );
  ...
  it("restores latest branch entry and filters missing tools", () => {
    const latest = latestStateFromEntries([
      {
        customType: "tool-manager:state",
        data: { active: ["missing", "read"], reason: "toggle", at: "a" },
      },
    ]);
    expect(latest?.active).toEqual(["missing", "read"]);
    expect(setAdditive([], latest?.active ?? [], resolveConfig(), tools)).toEqual([
      "read",
      SEARCH_TOOL_NAME,
    ]);
  });
