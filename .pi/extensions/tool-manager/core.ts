...
const LINEAR_WRITE_TOOLS = ["linear_create_issue", "linear_update_issue", "linear_create_comment"];
...
export function resolveConfig(settings: ToolManagerSettings = {}): ResolvedConfig {
  ...
  const defaultProfile =
    settings.defaultProfile && profiles[settings.defaultProfile] ? settings.defaultProfile : "core";
  return {
    defaultProfile,
    profiles,
...
export function unknownEntries(
  ...
  allTools: readonly Pick<ToolInfo, "name">[],
): string[] {
  const all = registeredNames(allTools);
  return unique(
    entries.filter((entry) => entry !== "*" && !config.groups[entry] && !all.has(entry)),
  );
}

export function expandRequestedEntries(
  entries: readonly string[],
  config: ResolvedConfig,
  allTools: readonly Pick<ToolInfo, "name">[],
): string[] {
...
function withLockedTools(
  names: readonly string[],
  config: ResolvedConfig,
  allTools: readonly Pick<ToolInfo, "name">[],
): string[] {
  const all = registeredNames(allTools);
  return unique([...names, SEARCH_TOOL_NAME, ...config.alwaysActive]).filter((name) =>
    all.has(name),
  );
}
...
export function profileTools(
  ...
  return withLockedTools(
    expandRequestedEntries(config.profiles[profile] ?? [], config, allTools),
    config,
    allTools,
  );
}

export function setAdditive(
...
export function searchTools(
  ...
  const groups = Object.entries(config.groups).filter(
    ([group]) => terms.includes(group) || (GROUP_HINTS[group] ?? []).some((rx) => rx.test(q)),
  );
  ...
  for (const tool of allTools) {
    ...
    if (!hasGroupHint) {
      ...
      const hits = terms.filter(
        (term) => name.split(/[_-]/).includes(term) || descriptionTerms.includes(term),
      );
  ...
  return {
    loaded,
    alreadyActive: matches.filter((name) => activeSet.has(name)),
    noMatch: matches.length === 0,
    groups: groups.map(([name]) => name).sort(),
  };
}
...
export function triggerGroups(
  ...
  const groups: string[] = [];
  const codingAction =
    /\b(fix|implement|edit|write|refactor|debug|review|change|patch|update|build|test|analy[sz]e)\b/.test(
      t,
    );
  const codeEntity =
    /\b(code|repo|repository|file|files|diff|worktree|pr|ci|\.([cm]?[jt]sx?|py|rs|go|java|sh|md|json|ya?ml|toml))\b/.test(
      t,
    );
  if (
    /\b(static checks?|code analysis|complexity|async-risk|duplicates? check)\b/.test(t) ||
    (codingAction && codeEntity)
  )
    groups.push("analysis");
  if (
    /\b(delegate|delegation|subagent|parallel agents?|background agent|start a subagent)\b/.test(t)
  )
    groups.push("delegation");
  if (
    /\b(create|build|review|reference) (a )?skill\b|\bskill (create|build|review|reference)\b/.test(
      t,
    )
  )
    groups.push("skills");
  if (/\b(catching|jit[- ]?catch)\b/.test(t)) groups.push("catching-tests");
  if (/\b(prior|previous|past) (session|conversation)\b|\bsession history\b/.test(t))
    groups.push("sessions");
  if ((GROUP_HINTS["pr-review"] ?? []).some((rx) => rx.test(t))) groups.push("pr-review");
  if (/https?:\/\/\S+|\b(web fetch|fetch (the )?(website|url|page)|website context)\b/.test(t))
    groups.push("web");
...
export function latestStateFromEntries(entries: readonly unknown[]): StateEntry | undefined {
  for (let i = entries.length - 1; i >= 0; i--) {
    ...
    if (
      ...
      entry !== null &&
      "customType" in entry &&
      (entry as { customType?: unknown }).customType === CUSTOM_TYPE
    ) {
      const data =
        (entry as { data?: unknown; details?: unknown }).data ??
        (entry as { details?: unknown }).details;
      if (
        typeof data === "object" &&
        data !== null &&
        Array.isArray((data as { active?: unknown }).active)
      )
