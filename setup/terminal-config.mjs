...
import { linkPathEffect, linked } from './file-ops.mjs';
...
function configureTmuxEffect(ctx, policy) {
  return Effect.sync(() => {
    ...
    if (existsSync(ctx.tmuxConf) && readFileSync(ctx.tmuxConf, 'utf8').includes(ctx.tmuxSourceLine)) {
...
function configureGhosttyEffect(ctx, policy) {
  return Effect.gen(function* () {
    ...
    if (ctx.env.SHOULD_LINK_GHOSTTY !== '1') {
      skip(`~/.config/ghostty (not needed for ${ctx.env.CONTEXT_LABEL ?? 'this context'}; set PI_ENV_LINK_GHOSTTY=1 to force)`);
      return;
    }
    ...
    yield* linkPathEffect(join(ctx.repo, 'ghostty/config'), join(ctx.ghosttyConfigDir, 'config'), '~/.config/ghostty/config');
    yield* linkPathEffect(join(ctx.repo, 'ghostty/themes/pi-env-gruvbox-dark'), join(ctx.ghosttyConfigDir, 'themes/pi-env-gruvbox-dark'), '~/.config/ghostty/themes/pi-env-gruvbox-dark');
    yield* linkPathEffect(join(ctx.repo, 'ghostty/themes/pi-env-gruvbox-light'), join(ctx.ghosttyConfigDir, 'themes/pi-env-gruvbox-light'), '~/.config/ghostty/themes/pi-env-gruvbox-light');
  });
}
