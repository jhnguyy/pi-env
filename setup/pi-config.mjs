...
import { appendOnceEffect, bootstrapFileEffect, linkPathEffect, linked } from './file-ops.mjs';
...
export function configurePiEffect(ctx) {
  return Effect.gen(function* () {
    ...
    yield* linkPathEffect(join(ctx.repo, '.agents/roles'), join(ctx.agentsDir, 'roles'), '~/.agents/roles');
    yield* linkPathEffect(join(ctx.repo, '.pi/extensions/__tests__/test-utils.ts'), join(ctx.testUtilsDir, 'test-utils.ts'), '~/.pi/agent/extensions/__tests__/test-utils.ts');
    yield* linkPathEffect(join(ctx.repo, '.pi/extensions/__tests__/loader.test.ts'), join(ctx.testUtilsDir, 'loader.test.ts'), '~/.pi/agent/extensions/__tests__/loader.test.ts');
    yield* appendOnceEffect(ctx.appendSrc, ctx.appendDst, ctx.appendMarker, '~/.pi/agent/APPEND_SYSTEM.md');
    yield* bootstrapFileEffect(
...
function applyManagedSettingsEffect(ctx) {
  return Effect.sync(() => {
    const result = ctx.run(ctx.setupNodeBin, ['setup/apply-managed-settings.mjs', ctx.settingsFile, ctx.managedSettingsFile, ctx.repo], { cwd: ctx.repo }).stdout.trim();
    switch (result) {
