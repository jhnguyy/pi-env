...
The check uses the installed Nub virtual store. It does not infer the artifact contents only from `package.json` or `lock.yaml`.
...
Review and update the policy only after you verify the new upstream terms and notice text.
...
- Corresponding-source references for MPL-covered packages.
- The Alpine package database summary and source build commits.
- The Node.js license from the pinned base image.

The Alpine package manifest identifies each package version, license expression, source origin, and build commit. The Alpine build recipe and upstream source provide the corresponding source for GPL-covered system packages.
