...
`analyzeEffect` is the typed core: configuration, scope, and Program setup failures remain in its Effect error channel. `analyze` is the reporting boundary and exhaustively converts those errors into the existing `AnalysisResult` failure shape. External analyzer and benchmark failures are captured individually so earlier findings and benchmark records survive.
