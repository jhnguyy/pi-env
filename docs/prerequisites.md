...
Nub is the canonical JavaScript toolchain boundary. Setup requires the exact Nub release in `package.json#packageManager` before dependency installation. A toolchain or frozen-lock failure preserves an existing `node_modules` tree.
