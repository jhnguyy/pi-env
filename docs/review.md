...
Generated comments include a visible AI disclosure. Findings with valid inline anchors become inline review comments. Unanchored findings go in the review body. The extension posts one GitHub review only after confirmation. It records the pending marker in review state and synchronizes a separate local posting-intent file before sending the request. The intent protects the remote side effect; it is not another store for findings or decisions. After an uncertain result, the extension searches GitHub for the marker. If no review is found, it blocks another POST until the result is verified. An unresolved attempt also blocks cleanup. If GitHub might have accepted a request, cancellation cannot undo it. The ordinary Pi session append does not guarantee durability for other review state after every partial storage failure.
