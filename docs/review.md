...
The harness writes each admitted raw finding to an immutable review-owned artifact. Parent state stores only its ID, role, evidence digest, verified artifact reference, retained raw ID references, and dismissal reasons. A requested raw finding is read through a 262,144-byte public reader that verifies review/run identity, containment, the no-symlink policy, actual byte length, digest, and decoded metadata before returning content. Invalid, incomplete, malformed, or unavailable consolidation degrades the review and falls back to one result finding per admitted raw occurrence. Fallback does not deduplicate identical findings and does not admit content from rejected reviewer outputs.
