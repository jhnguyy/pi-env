...
Generated comments include a visible AI disclosure. Findings with valid inline anchors become inline review comments. Unanchored findings go in the review body. The extension posts one GitHub review only after confirmation. A new Pi session must first write its review to JSONL; until then, posting asks you to wait for an assistant response. Posting also rejects session files above 128 MiB before creating an attempt. After confirmation, the extension checks and syncs the pending session entry before it records one local posting intent per review and sends the request. The intent remains outside review artifacts after cleanup. It protects the remote side effect, not findings or decisions. After an uncertain result, the extension searches GitHub for the marker before checking whether the current draft or remote head changed. A changed head still blocks a new POST, but it does not block reconciliation of an earlier attempt. If no review is found, the extension blocks another POST, including posts with changed content. An unresolved attempt also blocks cleanup, even when a legacy attempt has no intent file. Legacy pending attempts without an intent cannot be submitted again without separate reconciliation. If GitHub might have accepted a request, cancellation cannot undo it. A failed session write blocks further review mutations in that Pi process until restart; ordinary Pi session entries still do not guarantee durability after every partial storage failure.

The extension binds posting work to the parent session branch. A session change or tree navigation requests cancellation of local work and prevents a late result from changing the selected branch. A cancellation request alone does not establish that a provider stopped or that cleanup finished.
