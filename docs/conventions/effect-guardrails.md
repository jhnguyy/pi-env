...
- Expected operational failures use typed/tagged errors (`Data.TaggedError`, schema-backed tagged errors, existing domain tagged errors, or equivalent discriminated data).
...
- When relevant to the behavior, assert typed failures, interruption, finalization, retry bounds, and idempotency. Follow the repository-wide test classes and verification policy in [testing.md](testing.md).
